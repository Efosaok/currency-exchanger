...
const useCurrencyBlock = ({
  ...
  const convertFetch = (): Promise<ConvertCurrenciesResponse> =>
    axiosInstance.get(`/convert?${params}`).then((response) => response.data);

  const { data: convertData } = useQuery([from, to, amount], {
    enabled: false,
  });
  const canConvert = from && to && amount && convertData;

  const { data } = useQuery([from, defaultTargetCurrency, amount], {
